...
Object.entries(rails).forEach(([selector,items])=>{const section=document.querySelector(selector);if(!section)return;const wrap=document.createElement('div');wrap.className='swipe-module';wrap.innerHTML=`<div class="swipe-heading"><div><small>SWIPE TO EXPLORE</small><h3>更多成果与切片</h3></div><div class="swipe-controls"><button aria-label="向左">←</button><button aria-label="向右">→</button></div></div><div class="swipe-rail">${items.map(([num,title,copy,img])=>`<article class="swipe-card"><div class="swipe-image"><img src="${img}" alt="${title}" loading="lazy"></div><div class="swipe-copy"><strong>${num}</strong><h4>${title}</h4><p>${copy}</p></div></article>`).join('')}</div>`;section.append(wrap);const rail=wrap.querySelector('.swipe-rail'),[prev,next]=wrap.querySelectorAll('button');prev.onclick=()=>rail.scrollBy({left:-rail.clientWidth*.72,behavior:'smooth'});next.onclick=()=>rail.scrollBy({left:rail.clientWidth*.72,behavior:'smooth'});});
